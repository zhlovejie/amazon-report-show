...
function ReprotShow({ data, className ,repairDataList}: ReprotShowProps) {
  ...
  const getStickyStyle = (col: HeaderColumn, isHeader = false) => {
    ...
    return { backgroundColor: bg, position: "relative" as const, zIndex: 0 ,textAlign:col.align || 'left'};
  };
  //--table固定列功能-----------------------------------------------
    ...
    if(repairDataList.length > 0){
      ...
      setReportData(draft => {
        repairDataList.forEach(repairItem => {
          ...
          if(target){
            target[targetName] = D(target[targetName] as string).add(D(total)).toFixed(2)
          }else{
